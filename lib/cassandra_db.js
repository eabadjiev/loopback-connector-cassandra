...
cassandraDB.prototype.toFields = function (model, data, forCreate) {
  ...
  if (forCreate) {
    ...
    //my comment
    //var i=1
    for (var i = 0, len = nonIdsInData.length + props.ids.length; i < len; i++) {
      //my Comment
      query.push('?');
      if (i !== len - 1) {
        query.push(',');
...
function generateQueryParams(data, props,forupdate) {
  ...
  function pushToQueryParams_forAutoId(key) {
    var param = data[key] !== undefined ? data[key] : uniqueNo();
...
cassandraDB.prototype.executeSQL = function (sql, params, callback) {
  ...
  if( sql.indexOf("columnfamily_name")>0) {

   // var stmt = client.prepare(sql);
    client.execute(sql,params.params, function(err, rows) {
...
//my comment
      ...
      for(var i=0; i < rows.rows.length; i++) {
        var temp = {};
        temp['column'] = rows.rows[i].column_name;
       temp['type'] = mapCassandraColumnsType(rows.rows[i].validator);
      //  temp['type'] = rows.rows[i].validator;
        temp['nullable'] =  'NO';
       new_rows.push(temp);
      }
///end
    ...
    if( sql.startsWith('DROP') || sql.startsWith('CREATE') ) {
      sql = 'select key from system.local';
    }
...
function mapCassandraColumnsType(val)
{
  switch(val){
...
function getTableStatus(model, cb) {
  function decoratedCallback(err, data) {
    ...
    if (!err) {
      data.forEach(function (field) {
        //my comment
      //  field.type = mapCassandraDatatypes(field.type);
        field.type = mapCassandraColumnsType(field.validator);
  ...
  //sql="SELECT * FROM system.schema_columnfamilies WHERE columnfamily_name = ? AND keyspace_name=? ALLOW FILTERING;"

  sql="SELECT * FROM system.schema_columns  WHERE columnfamily_name = ? AND keyspace_name = ?;"
  var params = {}
  params.status = true;
  params.params=[model,this.settings.keyspace];
...
cassandraDB.prototype.alterTable = function (model, actualFields, cb) {
  var self = this;
...
function getColumnsToDrop(model, actualFields) {
  ...
  actualFields.forEach(function (actualField) {
    //my comment
    if (self.idColumn(model) === actualField.column_name) {
      return;
    }
    if (actualFieldNotPresentInModel(actualField, model)) {
      // my comment
     // sql.push('DROP COLUMN ' + self.escapeName(actualField.column_name));
     // sql.push('DROP ' + self.escapeName(actualField.column_name));
    }
  });
  if (sql.length > 0) {
    sql = [sql.join(', ')];
  ...
  function actualFieldNotPresentInModel(actualField, model) {
    //my comment
    return !(self.propertyName(model, actualField.column_name));
  }
}
...
function searchForPropertyInActual(model, propName, actualFields) {
  ...
  //  if (f.column === self.column(model, propName)) {
      if (f.column_name === self.column(model, propName)) {
      found = f;
      return;
    }
  });
...
function mapCassandraDatatypes(typeName) {
  return typeName;
}
